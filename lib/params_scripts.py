...
def ballot_size(PP):
    ...
    res = (
        3 * d * PP.q_bits
        + PP.k * PP.baselen * d * _bit_size(12 * PP.sigma1)
        + 256
...
def authority_size(PP):
    ...
    res = (
        2 * (PP.baselen * PP.amo_n * d * _bit_size(12 * PP.sigma2) + 256)
        + 2 * PP.baselen * d
        + (p - PP.Nv_max) * d * PP.q_bits
        + PP.Nv_max
    )
    return res
...
if __name__ == "__main__":
    ...
    for Nc in (32, 128, 1024):
        ...
        for i in range(1, maxi):
            ...
            print(
                f"Maximum voters={Nv_max:{maxi}}; ballot size = {(bs / (1024 * 8)).n()} KB, authority size = {(aus / (1024 * 8)).n()} KB"
            )
